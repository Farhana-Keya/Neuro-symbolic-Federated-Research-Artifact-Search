...
#
#     url = "https://resodate.org/resources/api/search/_search?pretty"
#
#     def __init__(self):
#         self.encoding = 'utf-8'
#
...
#     #     prompt = f"""
#     #             Extract the main key topics or subjects from the following question, focusing on essential nouns and named entities. Avoid generic terms such as 'impact', 'effect', or 'role.'
#     #
#     #             Question: "{question}"
#     #
#     #             Key topics:
#     #             """
#     #     # Call the OpenAI API to process the request with GPT-4, using the chat completion endpoint
...
#         """
#         Extracts key term from question, performs search on Resodate API,
#         and stores the results for each key term in a dictionary.
#         """
#         # Step 1: Extract key terms from the question
#         keyterm_extraction= Keyterm()
#         key_terms = keyterm_extraction.extract_key_term(question)
#         cleaned_user_type = user_type.split("Choice=")[1]
#         print("user type ",cleaned_user_type)
#         search_results = {}  # Dictionary to store results for each key term
#
#         # URL for the Resodate API
#
#         for key_term in key_terms:
#             print("Key term:", key_term)
#             if cleaned_user_type== "Dataset":
#             # Step 2: Set up the query payload with the key term
#                 query = {
#                     "size": 20,
#                     "from": 0,
#                     "query": {
...
#
#
#             # Step 3: Make the POST request to the Resodate API
#             headers = {
#                 'Content-Type': 'application/json',
#                 'accept': 'application/json'
#             }
#
#             response = requests.post(self.url, headers=headers, data=json.dumps(query))
#             data = response.json()
#             # Remove top-level keys you don’t want
#             keys_to_remove = ["took", "timed_out", "_shards","total", "value", "relation", "max_score","_index" ,"_type",  "_id"]
#             # Step 4: Check for errors in the response
#             for key in keys_to_remove:
#                 if key in data:
#                     del data[key]
#             if response.status_code == 200:
#                 # Step 5: Save the result for the current key term
#                 search_results[key_term] = data
#             else:
#                 print(f"API request failed for key term '{key_term}' with status code {response.status_code}")
#
